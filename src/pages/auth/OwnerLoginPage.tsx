...
export default function OwnerLoginPage() {
  ...
  useEffect(() => {
    ...
    if (session.isLoggedIn && session.workspace !== 'owner') {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    console.log('Owner login:', { email, password, workspace: workspaceInfo });
    
    // Set authentication state
    login('owner', email);
    
    // Redirect to dashboard after login
    navigate('/dashboard', { state: { workspace: workspaceInfo } });
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
            {/* Header */}
            <div className="p-6 bg-gradient-to-r from-blue-600/10 to-purple-600/10 border-b border-slate-800">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center">
                  <Shield className="w-5 h-5 text-white" />
                </div>
                <div>
                  <h1 className="text-xl text-white">Owner Access</h1>
                  <p className="text-sm text-slate-400">
                    {workspaceInfo?.workspaceName || 'Sign in as Owner'}
                  </p>
                </div>
              </div>
              {workspaceInfo && (
                <div className="px-3 py-2 bg-blue-600/20 border border-blue-500/30 rounded-lg">
                  <p className="text-xs text-blue-300">
                    You need Owner credentials to access this workspace
                  </p>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleLogin} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
                  required
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-lg text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-colors pr-12"
                    required
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white rounded-lg transition-all shadow-lg shadow-blue-500/25"
              >
                Sign In as Owner
              </button>
          ...
          <div className="mt-6 p-4 bg-slate-900/50 border border-slate-800 rounded-lg">
            <p className="text-xs text-slate-400 text-center">
              🔒 Owner accounts have full administrative privileges. Ensure you're on a secure network.
            </p>
          </div>
        </div>
      </div>
